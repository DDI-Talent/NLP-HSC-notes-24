...
So the `T1   PERSON 0 4   John` is actually `T1\tPERSON 0 4\tJohn`
...
# it's very simmilar to what we created in the previous badge.
...
gold_annotations <- read_brat_file("./data/gold.ann")
pred_annotations <- read_brat_file("./data/pred.ann")
```

## ACTIVITY: Manual evaluation

Inspect those two variables `gold_annotations` and `pred_annotations` in Environment. How many differences can you find? What types of differences are they?
...
```{r}
# Calculate metrics
metrics <- calculate_metrics(gold_annotations, pred_annotations)
